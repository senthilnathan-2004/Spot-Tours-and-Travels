...
<body style="margin: 0; padding: 0; background-color: #FFFFFF; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; color: #111827; -webkit-font-smoothing: antialiased;">
  <table width="100%" border="0" cellspacing="0" cellpadding="0" style="background-color: #FFFFFF;">
    <tr>
      <td align="center" style="padding: 28px 8px;" class="email-wrapper">
...
export async function sendBookingNotification(booking) {
  ...
  if (booking.email && booking.email.includes('@')) {
    ...
    await transporter.sendMail({
      from: FROM,
      to: booking.email,
      subject: `Booking Request Received: ${booking.packageTitle} (${booking.bookingRef})`,
      html: customerHtml
...
export async function sendBookingStatusNotification(booking, newStatus) {
  ...
  if (newStatus === 'confirmed') {
    config = {
      ...
      refId: booking.bookingRef,
      recipientGreeting: `Dear ${booking.fullName},`,
      messageIntro: `Great news! Your booking for <strong>${booking.packageTitle}</strong> starting on <strong>${booking.travelDate}</strong> is confirmed. Your verified hotel stays and dedicated tourist cab with an experienced polite chauffeur are reserved.`,
      specs,
      highlightBox: {
    ...
    config = {
      ...
      refId: booking.bookingRef,
      recipientGreeting: `Dear ${booking.fullName},`,
      messageIntro: `Thank you for choosing <strong>Spot Tours and Travels Coimbatore</strong> for your vacation. It was our pleasure serving you and your family. We hope your journey was comfortable, safe, and truly memorable.`,
    ...
    config = {
      ...
      specs: specs.slice(0, 4),
      highlightBox: {
        html: '<strong>Need Rescheduling or Custom Quote?</strong> Contact our Coimbatore team anytime to plan an alternate date or custom itinerary with zero extra fees.'
      },
  ...
  const subjectPrefix = newStatus === 'confirmed' ? 'Booking Confirmed' : newStatus === 'completed' ? 'Tour Completed' : 'Booking Cancelled';
  await transporter.sendMail({
    from: FROM,
    to: booking.email,
    subject: `${subjectPrefix}: ${booking.packageTitle} — ${booking.bookingRef}`,
    html
...
export async function sendEnquiryNotification(enquiry) {
  ...
  if (enquiry.email && enquiry.email.includes('@')) {
    ...
    await transporter.sendMail({
      from: FROM,
      to: enquiry.email,
      subject: `Spot Tours & Travels: We Received Your Enquiry for ${enquiry.destination || 'Your Vacation'}`,
      html: customerHtml
